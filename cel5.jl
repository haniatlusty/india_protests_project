...
# ╔═╡ f39f7106-ce3a-11ec-3f36-b3a2e9fd00ff
using StatsKit, Dates, Query, VegaLite, FileIO

# ╔═╡ b477653c-3457-4ca3-8d07-f6e6dcf1184f
database = CSV.read("./data/2016-2022.csv", DataFrame) 

# ╔═╡ 751bf1f4-de68-4680-b591-432d857d4360
unique(database.event_type)

# ╔═╡ 8d672c70-aecf-4d1b-95e5-78c2666a1610
unique(database.sub_event_type)

# ╔═╡ 8e5169e8-cf01-473f-8b9e-091c1c285225
function cyvil_violence(df)
	@from i in df begin
	    @where any(i.interaction .== [15 16 17 35 36 37])
	    @select {i.year, i.event_date, i.event_type, i.sub_event_type, i.location, i.latitude, i.longitude}
	    @collect DataFrame
	end
end

# ╔═╡ 60705001-349c-447f-b6da-ccb67eee89cc
df = cyvil_violence(database)

# ╔═╡ 8a95538c-3ab4-496d-b2b8-80c1e0317e96
df[!,"month"] = Int.(Dates.month.(Dates.Date.(df.event_date, "d U y")))

# ╔═╡ 782caaf6-e21f-40be-bb5e-250cbf2194b3
topo_json = read("./data/mapsindia1.json",String)

# ╔═╡ 5c9584fe-c946-4af2-8732-ab5f26fb27ae
dom = sort(unique(df.sub_event_type))

# ╔═╡ 473ab4a5-3cf8-4931-83f7-0aace26e1fb8
length(dom)

# ╔═╡ 98524a9b-602f-4d18-bcdb-46a6149ee9bd
colors = ["#069668", "#f90da0", "#35e0a9", "#c6523e", "#75d5e1", "#eb1138", "#347383", "#ccb0cb", "#476af9", "#e1c637", "#803ef3", "#f6a679", "#6e6494", "#fd95e8", "#926026"]

# ╔═╡ 7689de9a-cbc7-40a0-be11-fa6941fe2658
length(colors)

# ╔═╡ d6846739-fabb-43c6-90fd-4849347d822f
@vlplot(
    width = 900, 
    height= 700
) +
...
) |> save("./outputs/cel-5/wykres_mapa.png")

# ╔═╡ 1671ba25-a2b2-4c03-aa54-9b3e0a9e3c9e
@vlplot(
	width = 900, 
	height= 700
) +
...
) |> save("./outputs/cel-5/wykres_czas.png")

# ╔═╡ cc1b3554-4ad7-4df7-a31c-c93b9d40bd7b
for yr in [2016+(i-1)*2:2018+(i-1)*2 for i in 1:3]
	temp_data = @from i in df begin
			@where any(i.year .== yr)
			@select {i.year, i.month, i.sub_event_type, i.latitude, i.longitude}
			@collect DataFrame
	end
	temp_data[!,"month_period"] = (temp_data.year.-min(temp_data.year...))*12 .+ temp_data.month
	@vlplot(
	    width = 900, 
	    height= 700
	) +
	...
	@vlplot(
		...
		title      = "Mapa konfliktów z udziałem organów rządowych w latach $(yr[1])-$(yr[3])",
	) |> save("./outputs/cel-5/wykres_mapa_$(yr[1])_$(yr[3]).png")
	@vlplot(
	    width = 900, 
	    height= 700
	) +
	@vlplot(
		...
		title      = "Ilość konfliktów z udziałem organów rządowych  w latach $(yr[1])-$(yr[3])",
	) |> save("./outputs/cel-5/wykres_czas_$(yr[1])_$(yr[3]).png")
end
