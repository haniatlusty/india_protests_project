...
# ╔═╡ 08fa3de3-e685-47a1-8495-abdbc29c69ce
using StatsKit, Dates # statystyka (+CSV i DataFrames) i built-in do obsługi dat

# ╔═╡ 459ad7bd-0d37-428b-ad1b-fec111edbc9f
using VegaLite           # wykresy

# ╔═╡ 6235283c-126c-4b49-b649-906d516796f4
md"# Odczyt danych z bazy"

# ╔═╡ ad59402d-ab7a-45f5-8d9f-51e331697224
...
# ╔═╡ d185f094-aca3-42de-a192-d3983b39e92c
md"# Przykładowe przetworzenie danych"

# ╔═╡ a0b2623e-49a7-4be8-b6ae-e3dab344577b
function timeseries_gen(df::DataFrame, region::String, years::Tuple{Int,Int}, event_type::String = "any")
	...
	ts    = DataFrame(t = [keys(count)...], n = [values(count)...])
end

# ╔═╡ c57e5541-3e07-468b-a143-988c4939308a
ts = timeseries_gen(dataset, "Punjab", (2018,2018), "Protests")

# ╔═╡ ecd87ff8-9ad5-48ff-9797-134c8510a9c9
md"# Przykładowe wykresy"

# ╔═╡ 8c06c4f0-76d5-4259-9ef2-da37017e683c
@vlplot(
	:bar,
	height = 500,
	width  = 800,
...
# ╔═╡ e81e5539-0865-4959-9827-d48d13fa8f80
...
# ╔═╡ 73d14c2f-1176-42d9-8824-5af42941f7b7
function statecounter(df::DataFrame)
	counter = countmap(df.admin1)
	d = DataFrame(IDs = getIDs([keys(counter)...]), n = [values(counter)...], state = [keys(counter)...])
end

# ╔═╡ 6d5bd9d7-c20c-4995-8029-f6998d369880
statedata = statecounter(dataset)

# ╔═╡ a2f4eb7f-9a73-4cc9-a3ce-c1fadaa07a9a
topo_json = read("../data/mapsindia1.json", String)

# ╔═╡ 7a1e9da9-dc6f-47b3-9410-e87fed550544
@vlplot(
	height = 700,
	width = 900,
	data = {
