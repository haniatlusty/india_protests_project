...
# ╔═╡ a32f30de-ae42-4c9a-8530-5f2ba36ad82e
using PlutoUI            # interaktywne elementy zeszytu

# ╔═╡ e6175e9e-d099-11ec-0350-a31f4da3b508
using StatsKit, Dates # statystyka (+CSV i DataFrames) i built-in do obsługi dat

# ╔═╡ a6936a86-b7af-44b6-a13c-3e4dac0e5960
using VegaLite           # wykresy

# ╔═╡ 5d23d84e-04a4-4260-8c88-8f0f640b00d9
md"# Wykorzystane biblioteki"

# ╔═╡ 8b015fd7-4cc6-413c-9b3a-75dbe1110002
md"# Odczyt danych z bazy"

# ╔═╡ dee8dc52-e040-4b3a-9add-a6c5eb66f103
df = CSV.read(
		"../data/2016-2022.csv", 
...
# ╔═╡ 2bf63ef7-3018-42f8-ac52-16ac718c891f
md"# Przykładowe przetwarzanie danych"

# ╔═╡ 27c4ef91-8091-4179-8a7c-54b3840dd5a6
...
# ╔═╡ 67882160-9c67-468e-b9e7-a86f7efe906f
function filter_data(data::DataFrame, param::Symbol, value)
	df = subset(data, param => x -> x .== value)
end

# ╔═╡ fee13b68-92ff-44f0-8944-be278fc4ff93
df_2016 = filter_data(df, :year, 2016)

# ╔═╡ 23394ce9-41e0-43cc-b11a-21d0a1991870
function day_data(data::DataFrame, date::String)
	df = filter_data(data, :event_date, date)  # DataFrame dla konrketnego dnia
    n = nrow(df)                               # liczba zdarzeń danego dnia
    return (Set(df.event_type), Set(df.sub_event_type), Set(df.location), Set(df.fatalities), n, df)
end

# ╔═╡ c66fb94b-08ee-4d6d-a420-834bcffda9a1
date_conversion!(df, "day_of_week")

# ╔═╡ abac9cb0-bdb9-4ad2-b675-955274561d17
md"# Wykresy"

# ╔═╡ 06e223f5-c83c-4096-8eb3-e645a187b294
# wykres ilościowy od dni tygodnia
@vlplot(
    :bar,
	width  = 600,
	height = 400,
    ...
    x = {
		...
		axis = {title = "Dzień tygodnia"},
	},
    y = {
		"count()", 
		axis = {title = "Liczba protestów"},
	},
    color = {
		legend = false,
	},
    title = "Ilość zdarzeń na konkretny dzień tygodnia",
)

# ╔═╡ c6259ca8-5406-4083-bea8-e3aab43c550e
#Funkcja opisująca miesiąc i tworzaca do niego wykres
function dataplot_month(df::DataFrame, month::String)
	date_conversion!(df, "month")
	df        = df[df[!, :month] .== month , :]
	temp_data = date_conversion!(df, "day")
	years     = Set(temp_data.year)
	# title = 
	plot = @vlplot(
		:bar,
		width  = 700,
		...
		y = {
			"count()",
			title = "Liczba protestów",
		},
		title = length(years) > 1 ? month*" for several years" : month*" in "*string(years...),
		color = {
			legend = false,
		},
	)
    return temp_data, plot 
end

# ╔═╡ 3dab9f79-19d5-4108-83f6-de4f740735db
df_m, plot_2016 = dataplot_month(df_2016, "December");

# ╔═╡ 564b84a6-d8c8-4c78-8de6-c697ada43942
plot_2016

# ╔═╡ cf295ccc-d89d-478d-be8f-4121cee03f3f
df_m_all, plot = dataplot_month(df, "December");

# ╔═╡ cc202d6b-dd09-4dbb-8a7a-4f6c0dfad245
plot

# ╔═╡ f2649a82-1a4e-4d65-9f31-db2bf55c3105
md"# Mapy"

# ╔═╡ 408bc7ef-b5b5-4f62-bba8-ae6b36e316e6
states = read("../data/mapsindia1.json", String);

# ╔═╡ 98f4834c-2c0c-472b-a276-19d297ffb54d
@bind vals confirm(PlutoUI.combine() do Child
	md"""
	# Opcje wykresu
...
# ╔═╡ 76c61ab5-c0c4-407a-bb40-b5f2bad4edeb
...
@vlplot(
    ...
    data       = filter_data(df, :year, vals[1]),
    ...
    title      = "Mapa występowania konfliktów w roku $(vals[1])"
)
...
# ╠═f2649a82-1a4e-4d65-9f31-db2bf55c3105
# ╠═408bc7ef-b5b5-4f62-bba8-ae6b36e316e6
# ╠═c18b71af-fed7-4e33-b1b9-0f4eca0bbcb2
# ╟─80f53c82-6828-4207-b524-ba17af7e84e7
=======
# ╠═3dab9f79-19d5-4108-83f6-de4f740735db
# ╟─564b84a6-d8c8-4c78-8de6-c697ada43942
# ╠═cf295ccc-d89d-478d-be8f-4121cee03f3f
...
# ╟─f2649a82-1a4e-4d65-9f31-db2bf55c3105
# ╠═408bc7ef-b5b5-4f62-bba8-ae6b36e316e6
>>>>>>> 98f3b0b1b214f6bb3a38a5061a85238fb0c51829:prezentacja/prezentacja.jl
# ╟─98f4834c-2c0c-472b-a276-19d297ffb54d
# ╟─76c61ab5-c0c4-407a-bb40-b5f2bad4edeb
# ╟─00000000-0000-0000-0000-000000000001
# ╟─00000000-0000-0000-0000-000000000002
