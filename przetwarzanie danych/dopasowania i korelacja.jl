...
# ╔═╡ 861a4b00-dcee-11ec-143e-7923eb645233
using StatsKit, Dates # statystyka (+CSV i DataFrames) i built-in do obsługi dat

# ╔═╡ 7e3bb249-2a41-477c-bf9e-c0c52d16dd6d
using Plots, StatsPlots

# ╔═╡ 93ba2009-a6ab-4de7-8822-48ba6e790035
using TimeseriesPrediction

# ╔═╡ 9c01c2d6-4364-45b7-bd5b-b2a52f9d5434
md"## Odczyt danych"

# ╔═╡ 73d2a52d-ddf4-458f-8c45-7b8f04f58bb9
...
# ╔═╡ bab3084d-19d7-4840-b3c2-9c92e43e9724
md"## Przetworzenie danych"

# ╔═╡ 03acb7e1-03b5-4959-917d-9562bdb74e5d
...
# ╔═╡ 1e2b9149-617f-4ccb-ac1a-f1a020c89617
ts = timeseries_gen(dataset, "Punjab", (2016,2018), "Protests", true)

# ╔═╡ d73707b2-f176-4ecd-a7f3-c8b400306c9d
ts |> @df plot(:t,:n,dpi=600)

# ╔═╡ 4efef54a-d2db-486e-9792-dd42f6595ba0
md"## Dopasowanie"

# ╔═╡ a7690ba7-f155-4a42-abb8-e4684da9be77
model = loess(ts.t, ts.n, span=0.1);

# ╔═╡ 6a51f378-317d-4908-9318-97fd11648072
tp = range(extrema(ts.t)...; step = .1);

# ╔═╡ 9e8e4135-7c47-4322-a9f8-6321502a79fd
np = Loess.predict(model, tp);

# ╔═╡ 2f38095a-1c63-4c06-9ead-a6f64081971c
begin
	ts |> @df plot(:t,:n,dpi=600)
	plot!(tp,np)
end

# ╔═╡ 99f8ba21-4411-490c-aef4-62deb88484d7
dif = ts.n .- [np[1:10:end]...]

# ╔═╡ 183ffcba-3551-4d32-b34f-11f2fb0faf6e
plot(ts.t, dif)

# ╔═╡ f174f212-d8e1-4f58-9b8b-520b98b5b5a6
histogram(dif)

# ╔═╡ d193568b-ef2d-4da6-b51e-2e505d302adc
md"Ciekawy sposób predykcji do wdrożenia: [tutaj](https://datasciencejuliahackers.com/time-series.html#predicting-the-future)."

# ╔═╡ f20292f0-4fa7-4059-aa61-ee4692589470
md"""
### To do:
- wdrożenie predykcji z linku, albo biblioteki Timeseries
- wybranie danych do korelacji
- utworzenie modelu predykcyjnego na bazie skorelowanych danych
"""

# ╔═╡ f681aa61-9a77-42c4-8829-71a09bcd1318
timedata_fit = DataFrame(t = ts.t, n = np[1:10:end])

# ╔═╡ fbc11366-fd6a-4061-94af-300349d10d95
timedata_fit |> @df plot(:t,:n,dpi=600)

# ╔═╡ 51452878-3111-4f77-8023-ad00b9c2711c
fitted = fit_mle(Normal, dif)

# ╔═╡ be92be76-ff83-495a-bb3f-84707b547744
begin
	StatsPlots.plot(fitted)
end

# ╔═╡ 48b840ba-a530-431b-9c74-2fadd09a02d7
localmodel_tsp(ts.n, p=500; method = AverageLocalModel(ω_unsafe), ntype = FixedMassNeighborhood(2), stepsize = 1)
...
TimeseriesPrediction = "f218859d-9706-56aa-9ebf-1fa4ed7b8020"

[compat]
Plots = "~1.29.0"
StatsKit = "~0.3.1"
StatsPlots = "~0.14.32"
TimeseriesPrediction = "~0.7.0"
"""
...
git-tree-sha1 = "99bc204b5a729e86bb84302c528c0929d4a8ef77"
...
git-tree-sha1 = "a26aacf199c289e0162ca4cb4c9625e63d43e5ff"
...
[[deps.TimeseriesPrediction]]
deps = ["DelayEmbeddings", "IterTools", "LinearAlgebra", "MultivariateStats", "NearestNeighbors", "Parameters", "Reexport", "StaticArrays", "Statistics", "StatsBase"]
git-tree-sha1 = "2aed1b280428ceb8a1b3804e74c36003747e42af"
