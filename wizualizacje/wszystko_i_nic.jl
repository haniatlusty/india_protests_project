...
# ╔═╡ 08cfb970-ce41-11ec-37c7-45d284c31892
using StatsKit, Plots, Dates, VegaLite, VegaDatasets, Query, JSON

# ╔═╡ 3ea0be73-5c3e-4424-a0ca-77b3b09ebe48
df = CSV.read("C:\\Users\\kkarb\\OneDrive\\Pulpit\\projektpm\\data\\2016-2022.csv", select = ["location", "interaction", "longitude", "latitude", "data_id", "admin1"], DataFrame)

# ╔═╡ efdbf494-2579-4b85-8195-6db8fdce730a
length(unique(df.admin1))

# ╔═╡ aeefd430-a66a-4df0-9749-b3543a5bcfca
government_aggresion = [10, 11, 12, 13, 14, 15, 16, 17, 18, 30, 33, 34, 35, 36, 37, 38]

# ╔═╡ b71fa628-3983-4a3b-a7b3-9418d16ca26d
#tabelka z miejscami gdzie możliwa jest agresja ze strony państwa
conflict =  @from i in df begin                    
    @where i.interaction in government_aggresion
    @select {i.interaction, i.location, i.longitude, i.latitude, i.admin1}
    @collect DataFrame
end

# ╔═╡ 429bebdc-cffe-44e9-9a7d-5e7ed8449746
gdf = groupby(conflict, [:location, :longitude, :latitude])

# ╔═╡ c5242f28-1f13-4a2b-b39c-e471fb9ffe2e
combine_data = combine(gdf, nrow, :longitude, :latitude)

# ╔═╡ 2a7c9396-cc46-4d94-bc34-1f6e8f8c1d82
sorted_combine_data = sort!(combine_data, order(:nrow, rev = true))

# ╔═╡ e91870ca-65df-400f-b377-e9518c7004ab
ok_data = unique(sorted_combine_data)

# ╔═╡ be0ac672-fa98-4234-99ad-e2fc09cc3c83
most_aggresion = first(ok_data, 20)

# ╔═╡ a9abdd4d-24be-450e-8be0-e95466608529
most_aggresion.location

# ╔═╡ 63d54cb7-535a-40c2-bb20-81a69f2347fc
topo_json = read("..\\data\\mapsindia1.json", String)

# ╔═╡ 5b3ea85d-de15-421d-905b-8409641616ec
vlplot = @vlplot(
	height = 700,
	width = 900 ) +
@vlplot(
...
@vlplot(
    ...
    color = {
		:location,
        title = "Miasta" },
	title = "Mapa miejsc, które były najbardziej narażone na akty agresji ze strony	władzy"
...
#         type=:nominal,
# 		},
#     title = "Mapa miejsc, które były najbardziej narażone na akty agresji ze strony
# 	władzy" ) 

# ╔═╡ 06108300-ff5d-4d2e-8a7a-446d38f1fac0
#VegaLite.savefig("C:\\Users\\kkarb\\OneDrive\\Pulpit\\projektpm\\question41.png", vlplot)

# ╔═╡ 41dd90df-e43d-4002-8ca4-17d18ed18c1d
@vlplot(
	height = 700,
	width = 900 ) +
@vlplot(
...
# ╔═╡ 4b302d14-2748-46a4-b53d-bd2fe7c4ef0d
function getIDs(df::DataFrame, s::Symbol)
	...
	df[!,s] .|> k -> translation[k]
end

# ╔═╡ 2b5cbfdf-c3ac-4ced-8594-81e0c6dc27c5
df[!, "ID_admin1"] = getIDs(df, :admin1)

# ╔═╡ 2ee4ba5c-d3ac-4e76-a1d6-ef6cf98a0f5b
df

# ╔═╡ b512d2e2-c4f3-457c-ae74-598889726478
regiony_podzial = groupby(df, [:ID_admin1, :admin1])

# ╔═╡ 0c8e78d7-5553-4667-a5ef-bbd3dbab6d84
regiony = unique(combine(regiony_podzial, :ID_admin1))

# ╔═╡ fcc85ccd-387b-4b42-b8c6-b4f2f1c53f32
@vlplot(
	height = 700,
	width = 900 ) +
@vlplot(
...
# ╔═╡ 56c30f42-16b2-4f45-9560-5cd89087dd92
...
# ╔═╡ d941641f-4d6b-484d-aff6-689874553745
...
# ╔═╡ bb9db6ca-a341-447e-b00f-42c603aceae8
...
# ╔═╡ 66392083-7e09-4ce7-9815-955595385935
d = sort!(PleaseGodForgiveMeForThisAbomination(df), :state)

# ╔═╡ 4d852103-82c7-4ccd-93a8-0bbde59a49cd
mapka = @vlplot(
	height = 700,
	width = 900 ) +
@vlplot(
...
# ╔═╡ d8201dc8-3cb7-49b7-a4d4-d43a3d565860
#VegaLite.savefig("mapkafajna.png", mapka)

# ╔═╡ d19ddd54-baa3-4a57-b592-1fdb463175a6
#conflict
d_conflict = PleaseGodForgiveMeForThisAbomination(conflict)

# ╔═╡ e2f9eb98-4d05-4d04-bbd8-b5725bcbb7dc
mapka_władza = @vlplot(
	height = 700,
	width = 900 ) +
@vlplot(
    ...
    title = "Mapa przedstawiająca rejony narażone na agresję państwa"
) 

# ╔═╡ 23509d99-0ef5-4356-a268-3d1c69d84edb
#VegaLite.savefig("agresja_panstwa.png", mapka_władza)

# ╔═╡ 349bf1a3-57e0-48f8-9d87-ba8f43e35049
# population_data = CSV.read("C:\\Users\\kkarb\\OneDrive\\Pulpit\\populationCSV.csv", 
#     select = ["India / State/ Union Territory", "Population2011" ], DataFrame)
population_data = CSV.read("..\\data\\populationCSV.csv", 
   select = ["India / State/ Union Territory", "Population2011" ], DataFrame)

# ╔═╡ ec96769c-0c7d-49b1-bc1f-6a1e23135f7e
population_state = @view population_data[2:37, :]

# ╔═╡ 606eef27-2f49-47a1-871a-9d565cf2e02f
d[!, "Population"] = population_state.Population2011

# ╔═╡ 5dd6c331-9a61-41c8-842f-f9eb2e7e8656
d[!, "n/Population"] = d.n ./ d.Population

# ╔═╡ 77a4582e-d625-455c-b934-67dcf573de8e
d

# ╔═╡ a7e1c093-239f-4b3e-b486-10f3996de0f2
plot = @vlplot(
	height = 700,
	width = 900 ) +
@vlplot(
...
# ╔═╡ 262dc54b-f0fb-4596-8197-3c3f2a615dbc
VegaLite.savefig("C:\\Users\\kkarb\\OneDrive\\Pulpit\\protesty_per_ludnosc.png", plot)
